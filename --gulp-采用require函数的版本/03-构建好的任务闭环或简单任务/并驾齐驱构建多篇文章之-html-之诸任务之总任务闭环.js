...
const 构建一个任务闭环以将一Markdown文件转换成HTML文件
    = require('../02-用于构建任务闭环的工具集/用于构建效果呈现示例之文章的/构建一个任务闭环以创建效果呈现示例之文章')

const 构建简单任务用以将某示例性HTML配套的插图等资源复制到目标文件夹
    = require('../01-用于构建简单任务的工具集/构建简单任务用以将某示例性-html-配套的插图等资源复制到目标文件夹')
...
const 须囊括于本工具发布内容中之HTML范文之_凡配以亮色方案者_之存放文件夹之相对路径 = './文档/文章排版与配色效果示例集/以-html-格式渲染好的文章成品/html'
const 须囊括于本工具发布内容中之HTML范文之_凡配以暗色方案者_之存放文件夹之相对路径 = './文档/文章排版与配色效果示例集/以-html-格式渲染好的文章成品/html'
...
const 将所有须产出之HTML文件对应之插图等媒体资源复制到位之任务之本体函数 = 构建并行运转以下Gulp任务之总任务(
    ...[
...
const 并驾齐驱构建多篇文章之HTML之诸任务之总任务闭环 = create3HighOrderTasksUponMultipleTaskCycles({
