...
如果一个超链接指向**本文**某处，不妨称其为**文内链接**，而不应生硬地由英语直译为所谓“本地链接”。另，文内链接看起来与应与指向其它文章或网址的超链接稍有不同，以为明示。暗示不堪用，明示才好。小例一则：跳转至本文《[程序代码片段示例集](#程序代码片段示例集)》。
...
还有些内容会故意“杠掉”，即被中划线划去，表示此内容不正确，不应采纳，应作废。例如：~~地球是平的~~。
