import {
    发布的层叠样式表之根文件夹之相对路径 as _发布的层叠样式表之根文件夹之相对路径,
} from '../../本项目之全局配置项集'

export const 所涉一切文件甄选描述符之公共根文件夹之相对路径 = './源代码/原始的源代码/用于设计文章排版与配色方案的代码/stylus'
export const 发布的层叠样式表之根文件夹之相对路径 = _发布的层叠样式表之根文件夹之相对路径
export const 所有应用场景之入口Stylus文件相对路径之公共起始部分 = '逐个应用场景选用样式零件并装配成完整样式表'
export const 产出之一切层叠样式表文件之名称之共有前缀 = 'wulechuan-styles-for-html-via-markdown'

export const sharedSourceRelativeGlobs = []
export const extraSourceGlobsToWatch = []

export const 诸排版与配色方案之构建任务各自特有之配置之汇总列表 = [
    {
        // 应忽略该任务: true,
